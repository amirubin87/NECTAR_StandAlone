...
public class RunNectar_Weighted {
	...
	public static void main(String[] args) throws Exception {	
		if (args.length <2){
			...
			int index = 0;
			String pathToGraph = (args.length > index) ? args[index] : "C:/Temp/test1.txt";
			index++;
			String outputPath = (args.length > index) ? args[index] : "C:/Temp/";//"C:/Temp/WD/Chrome1_merge/";
			index++;			
			double[] betas =  (args.length > index) ? Utills.ParseDoubleArray(args[index]) : new double[]{1.0};		
			index++;
			...
			index++;
			boolean dynamicChoose =  false;//(args.length > index) ? Boolean.parseBoolean(args[index]) : false;
			index++;
			boolean useModularity =  true;//(args.length > index) ? Boolean.parseBoolean(args[index]) : true;
			index++;
			boolean useWOCC =  false;//(args.length > index) ? Boolean.parseBoolean(args[index]) : false;
			index++;
			boolean useConductance =  false;//(args.length > index) ? Boolean.parseBoolean(args[index]) : true;
			index++;
			...
			/*System.out.println("done");
			 BufferedReader br = new BufferedReader(new FileReader("C:/Temp/1.0.txt"));
			 String line = null;
